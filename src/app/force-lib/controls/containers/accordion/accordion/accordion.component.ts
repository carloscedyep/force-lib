...
export class AccordionComponent implements OnInit {
  ...
  abrirbody(id: string){
    ...
    const bodys = body.parentElement.parentElement.parentElement.querySelectorAll(".body");
    const labels = label.parentElement.parentElement.parentElement.querySelectorAll(".label");

    if (bodys.length > 0){
      var otherbodys = Array.prototype.filter.call(bodys, function(testElement){
    ...
    if (labels.length > 0){
      var otherlabels = Array.prototype.filter.call(labels, function(testElement){
        return testElement.id != id;
      });
