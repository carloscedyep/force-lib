...
export class CalendarComponent implements OnInit {
  ...
  changeDate(date: Date){
    ...
    this.monthName = date.toLocaleDateString('es-MX', this.options).toUpperCase();
  ...
  isToday(day: string){
    if (day != undefined && day.length > 0){
      var dayEval = new Date(this.date.getFullYear(), this.date.getMonth(), Number.parseInt(day));
      var today = new Date();
  ...
  selectDay(day: string){
    if (day!= undefined && day.length > 0){
      var newDate = new Date(this.date.getFullYear(), this.date.getMonth(), Number.parseInt(day));
      if(newDate){
        this.daySelect = new Date(this.date.getFullYear(), this.date.getMonth(), Number.parseInt(day));
