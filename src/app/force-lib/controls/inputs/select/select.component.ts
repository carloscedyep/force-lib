...
export class SelectComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.value != undefined && this.value.length > 0){
      const defaultItem = this.items.filter(i => i.value.includes(this.value));
    ...
    if (!this.isReadonly){
      ...
      if (menu){
        if (menu.getAttribute('expanded') == 'true'){
          ...
          this.positionService.setPosition(menu,leftText,bottomText,widthText,heightText, this.fixed)
        }
      }
    }
  }
